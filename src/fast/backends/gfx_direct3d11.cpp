...
void GfxRenderingAPIDX11::SetShadowMapParams(const float* viewProj, const float* splitDistances, int cascadeCount,
    ...
    if (captureCVars->GetInteger(SHADOW_MAP_CAPTURE_REQUEST_CVAR, 0) != 0) {
        ...
        try {
            ...
            // The ACTOR layer, which the first version of this capture did not write at all.
            //
            // Two things cast into these maps -- scenery and characters -- and the capture covered one.
            // That is not a small gap when the capture is the only way to study a reported artefact
            // offline: a defect in a character's shadow can be looked for at length in a file that
            // provably does not contain it.
            //
            // The actor slices sit right after the world ones in mShadowMapTexture while the two layers
            // share a resolution, and in mShadowActorTexture when they do not, so both arrangements are
            // written and the metadata says which one produced the file.
            ...
            if (actorCount > 0) {
                ...
                if (mShadowActorTexture != nullptr) {
                    WriteShadowDepthCapture(mDevice.Get(), mContext.Get(), mShadowActorTexture.Get(),
                                            actorCount, actors);
                } else {
...
#define CAPTURE_FIELD(field) metadata[#field] = mPerShadowCbData.field
            ...
            // The actor layer's own bounds and texel size, captured because the actor layer itself now is.
            CAPTURE_FIELD(shadow_actor_min);
            CAPTURE_FIELD(shadow_actor_max);
            CAPTURE_FIELD(shadow_actor_texel_uv);
#undef CAPTURE_FIELD
            // Every float4 the receiver reads is above, and this list is hand-written, so it drifts
            // silently: a field was once added to the buffer and not here, and the first capture taken
            // afterwards could not say whether that technique had been on. A reproduction built from a
            // capture that omits one of the kernel's inputs is not a reproduction of anything. This
            // assert is what makes the next omission loud instead of silent.
            static_assert(sizeof(PerShadowCB) ==
                              sizeof(float[SHADOW_MAP_MAX_CASCADES][16]) + 17 * sizeof(float[4]),
